...
database.prototype.addUser = function(username, password, call){
	this.db.run("INSERT INTO Citizens (username,password,onoff,accountstatus,level) VALUES (?,?,?,?,?)",username,password,"offline", "active", "Citizen",call("User created"));
},
...
database.prototype.getAdminList = function(call){
    var query = "SELECT * FROM Citizens ORDER BY username;";
    this.db.all(query, function(err, rows) {
	if (err)
	    console.log(err);
	call(rows);
...
database.prototype.getAdminUser = function(username, call){
    var query = "SELECT * FROM Citizens WHERE username ='" + username + "';";
    this.db.all(query, function(err, rows) {
	if (err)
	    console.log(err);
	call(rows);
